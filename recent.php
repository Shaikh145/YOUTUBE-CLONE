...
// Get recent videos (most recently uploaded)
try {
...
<head>
    ...
    <title>Recently Uploaded - YouTubeClone</title>
...
<body>
    ...
    <div class="categories">
        ...
        <div class="category" onclick="location.href='trending.php'">Trending</div>
        <div class="category active">Recently Uploaded</div>
    ...
    <h2 class="section-title">Recently Uploaded Videos</h2>
    
    <div class="videos-container">
        <?php if(empty($videos)): ?>
            <div class="no-videos">No videos available yet.</div>
        <?php else: ?>
            <?php foreach($videos as $video): ?>
                <?php
                    // Check if video is recent (less than 3 days old)
                    $upload_time = strtotime($video['upload_date']);
                    $current_time = time();
                    $is_recent = ($current_time - $upload_time) < (3 * 24 * 60 * 60); // 3 days in seconds
                ?>
                <div class="video-card" onclick="location.href='watch.php?id=<?php echo $video['id']; ?>'">
                    <?php if($is_recent): ?>
                        <div class="new-tag"><i class="fas fa-clock"></i> New</div>
                    <?php endif; ?>
